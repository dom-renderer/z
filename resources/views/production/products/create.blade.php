...
<div class="bg-light p-4 rounded">
    <div class="container mt-4">
        <form method="POST" action="{{ route('production.products.store') }}">
            @csrf
            ...
            <div class="mb-3">
                ...
                <input type="text" name="sku" class="form-control" value="{{ old('sku') }}" placeholder="Enter SKU" required>
                @if ($errors->has('sku'))
            ...
            <div class="mb-3">
                ...
                <select name="uom_ids[]" class="form-control select2" required multiple>
                    @foreach($uoms as $uom)
                        <option value="{{ $uom->id }}" @if(collect(old('uom_ids', []))->contains($uom->id)) selected @endif>{{ $uom->code }} - {{ $uom->name }}</option>
                    @endforeach
                </select>
